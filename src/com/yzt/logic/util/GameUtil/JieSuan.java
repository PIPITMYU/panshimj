...
public class JieSuan {
	public static void xiaoJieSuan(String roomId) {
		...
		//获取玩家的胡分
 		if(room.getHuangZhuang() != null && room.getHuangZhuang() == true){//荒庄--庄不变,计算杠分
 			...
 			for (Player player1 : players) {
 				//更改玩家的分数
 				player1.setScore(player1.getScore()+player1.getaGangScore()+player1.getmGangScore());
 			}		
		}else{
			...
			// 获取玩家的胡的类型
			List<Integer> huInfoList = MahjongUtils.checkHuInfo(winPlayer,room); 
			//获取胡的牌型分
			...
			if (room.getScoreType() == 1) {
				if(zhuang){ //赢家是庄
					if(ziMo){ //赢家是庄  自摸
						for(Player p: players){
							if(p.getIsHu()){ //基础分*3家*庄家*自摸
								...
								p.setThisScore(huScore+p.getaGangScore()+p.getmGangScore());
					...
					if(ziMo){ //赢家不是庄，点炮三家付,是自摸
						...
						winPlayer.setThisScore(winPlayer.getHuScore()+winPlayer.getaGangScore()+winPlayer.getmGangScore());
				...
				if(zhuang){ //赢家是庄
					if(ziMo){ //赢家是庄  自摸
						...
						for(Player p: players){
							if(p.getIsHu()){ 
								p.setHuScore(huScore);
								p.setThisScore(p.getHuScore()+p.getaGangScore()+p.getmGangScore());
								p.setScore(p.getThisScore() + p.getScore());
							}else{
								if(p.getIsDian()){//包三家胡分--改了1：不包含铺分--又改了2：包铺分
									...
									p.setThisScore(p.getaGangScore()+p.getmGangScore());
									p.setScore(p.getThisScore() + p.getScore());
								}
							}
						}
		...
		for (Player p : players) {
			...
			map.put("gangScore", p.getaGangScore()+p.getmGangScore());
