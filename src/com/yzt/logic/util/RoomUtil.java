		...
		if (room.getXiaoJuNum() != null && room.getXiaoJuNum() != 0) {
			...
			if (room.getLastNum() != 0) {
				...
				for (Player player : players) {
					player.setScore(player.getScore()+player.getmGangScore()+player.getaGangScore());
				}
			}
			...
			if (room.getState() == Cnst.ROOM_STATE_GAMIING) {
				...
				for (int i = 0; i < room.getPlayerIds().length; i++) {
					xiaoJSInfo.add(players.get(i).getaGangScore()+players.get(i).getmGangScore());
				}
			...
			// 开房选项
			roomSave.put("fengDingFen", String.valueOf(room.getFengDingFen()));
			roomSave.put("playerNum", String.valueOf(room.getPlayerNum()));
			roomSave.put("queMen", String.valueOf(room.getQueMen()));
			// 房间规则
	...
	public static void addRoomToDB(RoomResp room) {
		...
		taskExecuter.execute(new Runnable() {
			public void run() {
				...
				UserMapper.updateMoney(UserMapper.getUserMoneyByUserId(createId) - needMoney, createId + "");
			}
		});
		// 添加玩家消费记录
	...
	private static void addClubTODB(RoomResp room) {
		...
		taskExecuter.execute(new Runnable() {
			public void run() {
				...
				ClubMapper.updateClubMoney(room.getClubId(), needMoney);
			}
		});
		Integer needMoney = 0;
		Integer circleNum = room.getCircleNum();
