...
public class HallFunctions extends TCPGameFunctions {
	...
	public synchronized static void interface_100007(WSClient channel, Map<String, Object> readData) {
		...
		Integer scoreType = StringUtils.parseInt(readData.get("scoreType"));//计分方式： 1点炮三家付  2 点炮包三家

		Integer playerNum = StringUtils.parseInt(readData.get("playerNum"));// 玩家人数 2 3 4
		Integer queMen = StringUtils.parseInt(readData.get("queMen"));// 是否缺门：1 可以缺   2不可以缺
		Integer fengDingFen = StringUtils.parseInt(readData.get("fengDingFen"));// 封顶分： 32 64 128
		...
		// 检测需要的房卡
		Integer needMoney = 0;
		if (circleNum == 8) {// 30张的时候，房卡扣双倍
			needMoney = 2 * playerNum;
		} else {// 按照选的人数扣除房卡
			needMoney = playerNum;
		}
		...
		room.setQueMen(queMen);
		room.setFengDingFen(fengDingFen);
		// 初始化大接口的id
		...
		if (roomType == null) {
			...
			// 房主创建的房间,人数座位座位都是6人的
			room.setInitPositions(MahjongUtils.initPosition(4));
			// 设置用户信息
			...
			userIds = new Long[playerNum];
			// 将新创建的房间加入到代开房间列表里面
			room.setInitPositions(MahjongUtils.initPosition(circleNum));
	...
	public synchronized static void interface_100008(WSClient channel, Map<String, Object> readData) {
		...
		if (jionIndex == room.getPlayerNum()) {// 加入的人数已经等于房间的最大人数
		...
		//规则
		info.put("queMen", room.getQueMen());
		info.put("fengDingFen", room.getFengDingFen());
		...
		if (hgetAll != null && hgetAll.size() > 0) {
			...
			for (String roomId : keySet) {
				...
				if (room != null && room.getCreateId().equals(userId) && room.getState() != Cnst.ROOM_STATE_YJS && room.getRoomType() == Cnst.ROOM_TYPE_2) {
					...
					//规则
					map.put("queMen", room.getQueMen());
					map.put("fengDingFen", room.getFengDingFen());
	...
	public static void interface_100013(WSClient channel, Map<String, Object> readData) {
		...
		Integer playerNum = room.getPlayerNum();
		if (circleNum == 8) {// 30张的时候，房卡扣双倍
			needMoney = 2 * playerNum;
		} else {// 按照选的人数扣除房卡
			needMoney = playerNum;
